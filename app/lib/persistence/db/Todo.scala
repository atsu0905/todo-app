...
case class TodoTable[P <: JdbcProfile]()(implicit val driver: P)
  ...
  class Table_category(tag: Tag) extends BasicTable(tag, "to_do_category") {
    ...
    def * = (id.?, name, slug, color, updatedAt, createdAt) <> (
      // Tuple(table) => Model
      (t: TableElementTuple) => Todo_category(
